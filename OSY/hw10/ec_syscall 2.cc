...
void Ec::syscall_handler (uint8 a)
{
    ...
    switch (number) {
        ...
        case sys_break: {
            ...
            mword cur_pages_break = (Ec::break_current % PAGE_SIZE != 0) ? Ec::break_current - (Ec::break_current % PAGE_SIZE) + PAGE_SIZE : Ec::break_current;
            mword new_pages_break = (address % PAGE_SIZE != 0) ? address - (address % PAGE_SIZE) + PAGE_SIZE : address;
            mword offset;
            
            // alloc pages
            if (address > Ec::break_current){
                mword orig_pages_break = cur_pages_break;
                // reset the unused bites of page
                if ((offset = Ec::break_current % PAGE_SIZE) != 0){
                    void *current_page = Kalloc::phys2virt(Ptab::get_mapping(cur_pages_break - PAGE_SIZE) & ~PAGE_MASK);
                    memset(reinterpret_cast<void*>(reinterpret_cast<mword>(current_page) + offset), 0, PAGE_SIZE - offset);
                }
                // allocation
